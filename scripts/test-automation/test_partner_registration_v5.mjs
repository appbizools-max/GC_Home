...
async function runTests() {
  ...
  const { data: activeServices, error: err1 } = await supabase
    .from('services')
    .select('id, name, category, category_id, is_active, starting_price, estimated_duration')
    .eq('is_active', true)
    .order('name', { ascending: true });

  assert(!err1 && Array.isArray(activeServices) && activeServices.length > 0,
  ...
  const { data: allServices, error: err2 } = await supabase
    .from('services')
    .select('id, name, is_active');

  assert(!err2, 'Fetched all services for active flag comparison');
  ...
  // Verify active query contains ZERO inactive services
  ...
  const { data: insertedMaid, error: insertErr } = await supabase
    .from('maid_profiles')
    .insert([testPartnerPayload])
    .select('id, maid_code, full_name, city, service_radius_km, services_provided, bank_name, status')
    .single();

  assert(!insertErr && insertedMaid?.maid_code === testPartnerCode,
  ...
  const { data: adminFetched, error: adminErr } = await supabase
    .from('maid_profiles')
    .select('*')
    .eq('maid_code', testPartnerCode)
    .single();

  assert(!adminErr && adminFetched !== null, 'Admin Panel can query the new partner application');
  ...
  assert(!delErr, `Test partner ${testPartnerCode} cleaned up cleanly`);
